...
export function LandingPage() {
  ...
  const [activeTab, setActiveTab] = useState<'courses' | 'onboarding' | 'stats'>('courses');
  ...
  return (
    <div className="bg-[#f8fafc] text-slate-900 antialiased overflow-x-hidden subtle-dot-grid">
      {/* ── CURSOR SPOTLIGHT ── */}
      <div
      ...
      <section className="min-h-screen flex items-center justify-center text-center px-8 pt-16">
        <motion.div style={{y: heroY}} className="w-full max-w-4xl mx-auto">
          ...
          >
            <h1 className="font-extrabold text-slate-900 text-[clamp(52px,7vw,88px)] leading-[1.1] tracking-[-0.045em] mx-auto">
              Обучение команды,{' '}
            ...
            <p className="text-slate-500 text-[clamp(16px,1.6vw,20px)] leading-[1.7] max-w-2xl mx-auto">
              Курсы с тестами, онбординг новых сотрудников и контроль успеваемости — полный цикл корпоративного обучения в одной системе.
            </p>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-[0_8px_48px_rgba(0,0,0,0.08)] overflow-hidden">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/60">
                ...
                <div className="flex items-center bg-white p-1 rounded-xl border border-slate-100 shadow-xs gap-0.5">
                  {(['courses', 'onboarding', 'stats'] as const).map((tab) => (
              ...
              <div className="p-8 min-h-[300px] flex flex-col justify-between">
                <AnimatePresence mode="wait">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="md:col-span-2 p-5 border border-blue-100 bg-blue-50/40 rounded-xl">
                          <div className="flex items-start justify-between gap-4 mb-4">
                            ...
                            <span className="text-[11px] text-slate-400 font-medium shrink-0 mt-0.5 bg-white px-2 py-1 rounded-lg border border-slate-100">3 из 4 уроков</span>
                          </div>
                        ...
                        <div className="flex flex-col gap-3">
                          <div className="flex items-center gap-3 p-4 border border-slate-100 rounded-xl bg-white">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-300 mt-0.5">Завершён · сертификат выдан</p>
                            </div>
                          </div>
                  ...
                  {activeTab === 'onboarding' && (
                    <motion.div key="o" initial={{opacity: 0, y: 4}} animate={{opacity: 1, y: 0}}
                                exit={{opacity: 0, y: -4}} transition={{duration: 0.16}}>
                      <div className="flex items-center justify-between mb-6">
                        <div>
                          <p className="text-[11px] text-slate-400 font-medium uppercase tracking-widest mb-1">Онбординг</p>
                          <h4 className="text-[16px] font-semibold text-slate-900">Вступление в должность</h4>
                        </div>
                        <p className="text-[22px] font-bold text-slate-900 leading-none">2 <span className="text-[14px] text-slate-300 font-normal">/ 5 шагов</span></p>
                      </div>
                      <div className="space-y-2.5">
                        {[
                          {t: 'Ознакомиться с регламентом', s: 'done', type: 'document'},
                          {t: 'Встреча с руководителем', s: 'done', type: 'meeting'},
                          {t: 'Корпоративная культура и ценности', s: 'active', type: 'course'},
                          {t: 'Настройка рабочего места', s: 'todo', type: 'task'},
                          {t: 'Итоговое собеседование', s: 'todo', type: 'meeting'},
                        ].map((step, i) => (
                          <div key={i}
                            ...
                            <span className={`text-[13.5px] flex-1 ${step.s === 'todo' ? 'text-slate-300' : step.s === 'active' ? 'text-slate-900 font-medium' : 'text-slate-500'}`}>{step.t}</span>
                            <span className={`text-[10px] font-medium px-2 py-0.5 rounded-md ${step.s === 'done' ? 'text-slate-400 bg-slate-50' : step.s === 'active' ? 'text-blue-600 bg-blue-50 font-semibold uppercase tracking-widest' : 'text-slate-200'}`}>
                              {step.s === 'done' ? 'готово' : step.s === 'active' ? 'сейчас' : step.type === 'course' ? 'курс' : step.type === 'meeting' ? 'встреча' : 'задача'}
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  {activeTab === 'stats' && (
                    <motion.div key="s" initial={{opacity: 0, y: 4}} animate={{opacity: 1, y: 0}}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
                        {[
                          {l: 'Завершено', v: '4', sub: 'курса'},
                          {l: 'Сертификатов', v: '4', sub: 'получено'},
                          {l: 'В процессе', v: '2', sub: 'курса'},
                          {l: 'Ср. прогресс', v: '68%', sub: 'по всем'},
                        ].map((s, i) => (
                      ...
                      <div className="flex items-end gap-1.5 h-14 p-4 border border-slate-100 rounded-xl bg-white">
                        {[30, 50, 42, 68, 80, 92, 75, 100].map((v, i) => (
                          <div key={i} className="flex-1 bg-slate-100 rounded overflow-hidden h-full relative">
                            <div className="absolute bottom-0 inset-x-0 bg-blue-600/25 rounded" style={{height: `${v}%`}}/>
                          </div>
                        ))}
                      </div>
                    </motion.div>
            ...
            {[
              {
                num: '01',
                title: 'Курсы',
                desc: 'Модули, уроки в Markdown, тесты с порогом прохождения. Автоматические сертификаты при 100% прохождении.'
              },
              {
                num: '02',
                title: 'Назначение',
                desc: 'Руководитель записывает сотрудника напрямую — курс сразу доступен. Или сотрудник подаёт заявку и ждёт одобрения.'
              },
              {
                num: '03',
                title: 'Онбординг',
                desc: 'Шаги адаптации нового сотрудника: задачи, документы, встречи, видео, курсы. Встроенный чат с куратором.'
              },
              {
                num: '04',
                title: 'Контроль',
                desc: 'Статистика по отделам и сотрудникам. Просмотр по курсу или по человеку. Фильтры по департаменту и статусу.'
              },
            ].map((f, i) => (
        ...
        <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          <FadeUp className="lg:col-span-4">
            ...
            <p className="text-[16px] text-slate-400 leading-relaxed">
              Пять ролей с разграниченным доступом. Скоуп статистики и управления зависит от уровня в структуре компании.
            </p>
          </FadeUp>
          ...
          <div className="lg:col-span-8 divide-y divide-slate-100">
            {[
              {
                role: 'Администратор',
                desc: 'Полный доступ: управление курсами, одобрение публикаций, статистика по всей компании',
                badge: 'Полный доступ'
              },
              {
                role: 'Руководитель департамента',
                desc: 'Создаёт курсы, назначает обучение и онбординги, видит статистику своего департамента',
                badge: null
              },
              {
                role: 'Руководитель отдела',
                desc: 'Создаёт курсы, управляет обучением и онбордингом внутри своего отдела',
                badge: null
              },
              {
                role: 'Старший менеджер',
                desc: 'Назначает курсы и онбординги, видит прогресс по своим назначениям',
                badge: null
              },
              {
                role: 'Менеджер',
                desc: 'Проходит курсы, подаёт заявки на обучение, получает и проходит онбординг',
                badge: 'Сотрудник'
              },
            ].map((r, i) => (
            ...
            {[
              {
                ...
                a: 'Администратор создаёт корпоративный аккаунт и передаёт данные сотруднику. Все пользователи — сотрудники компании.'
              },
              {
                q: 'Как попасть на курс?',
                a: 'Два пути: руководитель назначает курс напрямую — он сразу становится доступным. Либо сотрудник сам подаёт заявку, которую нужно одобрить.'
              },
              {
                q: 'Как выдаются сертификаты?',
                a: 'Автоматически при 100% прохождении курса. Сертификат отображается в профиле — его можно открыть и распечатать.'
              },
              {
                q: 'Что видит руководитель в разделе «Контроль»?',
                a: 'Зависит от роли. Администратор — всех сотрудников. Руководитель департамента — свой департамент. Руководитель отдела — свой отдел. Старший менеджер — только тех, кому лично назначал курсы.'
              },
            ].map((item, i) => {
      ...
      <section className="min-h-screen relative overflow-hidden bg-slate-950 subtle-dot-grid-dark flex items-center justify-center">
        ...
        >
          ...
          <div className="mt-2 pt-2 border-t border-white/10 flex items-center justify-between">
            <span className="text-[9px] text-white/25">09.06.2026</span>
        ...
        >
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Онбординг · 2 из 5</p>
          </div>
          {['Ознакомление с регламентом', 'Встреча с руководителем', 'Корпоративный курс'].map((t, i) => (
