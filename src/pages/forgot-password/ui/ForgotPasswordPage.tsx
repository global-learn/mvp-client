...
export function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Бэкенд отвечает одинаково независимо от наличия аккаунта (анти-энумерация)
      setSent(true);
    } catch (err) {
      setError(getApiErrorMessage(err, 'Не удалось отправить письмо. Попробуйте позже'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        {sent ? (
          <div className="text-center">
            ...
            <p className="text-[14px] text-slate-500 leading-relaxed">
              Если аккаунт с адресом <b>{email}</b> существует, мы отправили на него ссылку для сброса пароля.
            </p>
            <Link
              to="/login"
